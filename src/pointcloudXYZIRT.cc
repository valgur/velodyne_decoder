...
PointcloudXYZIRT::PointcloudXYZIRT(const double max_range, const double min_range,
                                   const std::string &target_frame, const std::string &fixed_frame,
                                   const unsigned int scans_per_block)
    : DataContainerBase(
          max_range, min_range, target_frame, fixed_frame, 0, 1, true, scans_per_block, 6, "x", 1,
      ...
      iter_x(cloud, "x"), iter_y(cloud, "y"), iter_z(cloud, "z"), iter_ring(cloud, "ring"),
...
void PointcloudXYZIRT::addPoint(float x, float y, float z, uint16_t ring, uint16_t /*azimuth*/,
